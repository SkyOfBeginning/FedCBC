...
from data_process.utils import build_transform
...
class cifar100_Data_Spliter():
    ...
    def train_feature_extractor(self):
        trans = build_transform(True, self.input_size)
        self.cifar100_dataset = CIFAR100(root='./dataset', train=True, download=True, transform=trans)
        ...
        #
        class_counts = torch.zeros(100)  # 每个类的数量
        class_label = [[] for i in range(100)]  # 每个类的index
        j = 0
        for x, label in tqdm(self.cifar100_dataset):
            label = int(label)
            class_counts[label] += 1
            class_label[label].append(j)
            j += 1
        ...
        for j in range(100):
            ...
            while q < 30:
                ...
                unused_indice.remove(random_index)
                q += 1
        subset.append(Subset(self.cifar100_dataset,index))
        subset = subset[0]

        trainloader = DataLoader(subset,batch_size=32,shuffle=True)

        test_data = CIFAR100(root='./dataset', train=False, download=True, transform=trans)
        ...
        torch.save(self.feature_extractor,'./pretrain_models/resnet-forCIFAR100-30.pth')
